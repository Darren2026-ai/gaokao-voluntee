...
class GaoKaoAnalyzer {
  ...
  getCollegeList() {
    ...
    // 确保至少有15个推荐（不足时自动扩大范围补充）
    if (total < 15) {
      console.warn('[getCollegeList] ⚠️ 推荐数(' + total + ')不足15个，启动扩大范围兜底...');
      return this._getCollegeListExpanded(score, rank, stayGuangdong, targetMajor, priority);
    }
  ...
  // 兜底：扩大阈值范围，确保生成足够的推荐
  _getCollegeListExpanded(score, rank, stayGuangdong, targetMajor, priority) {
    console.log('[getCollegeList-expanded] 🔄 使用扩大阈值重新生成...');

    // 大幅扩大范围，分多轮补充
    const expandedThresholds = [
      { chongMin: -15000, wenMin: -5000, wenMax: 8000, baoMin: 8000, baoMax: 25000 },
      { chongMin: -20000, wenMin: -8000, wenMax: 12000, baoMin: 12000, baoMax: 35000 },
      { chongMin: -25000, wenMin: -12000, wenMax: 18000, baoMin: 18000, baoMax: 50000 },
      { chongMin: -30000, wenMin: -18000, wenMax: 25000, baoMin: 25000, baoMax: 70000 },
      { chongMin: -40000, wenMin: -25000, wenMax: 35000, baoMin: 35000, baoMax: 90000 },
    ];

    const chong = [], wen = [], bao = [];

    for (const thresholds of expandedThresholds) {
      if (chong.length >= 10 && wen.length >= 12 && bao.length >= 8) break;
      ...
      if (typeof COLLEGES !== 'undefined' && COLLEGES && Array.isArray(COLLEGES)) {
        COLLEGES.forEach(college => {
          ...
          college.groups.forEach(group => {
            if (!group || group.minRank === undefined || group.minScore === undefined) return;

            const diff = group.minRank - rank;
            let majorMatch = false;
            if (targetMajor && group.subjects && Array.isArray(group.subjects)) {
              majorMatch = group.subjects.some(s =>
                (s && s.includes(targetMajor.slice(0, 4))) || (targetMajor && targetMajor.includes(s?.slice(0, 3)))
              );
            }
            const priority_score = this.calcPriorityScore(college, group, priority);

            // 避免重复添加
            const id = `${college.id}_${group.code}`;
            const alreadyIn = [...chong, ...wen, ...bao].some(x => x.id === id);
            if (alreadyIn) return;

            if (diff < 0 && diff > thresholds.chongMin && chong.length < 10) {
              const item = this.buildRecommendItem(college, group, score, rank, 'chong', majorMatch, priority_score);
              if (item) chong.push(item);
            } else if (diff >= thresholds.wenMin && diff <= thresholds.wenMax && wen.length < 12) {
              const item = this.buildRecommendItem(college, group, score, rank, 'wen', majorMatch, priority_score);
              if (item) wen.push(item);
            } else if (diff > thresholds.baoMin && diff <= thresholds.baoMax && bao.length < 8 && college.level !== '专科') {
              const item = this.buildRecommendItem(college, group, score, rank, 'bao', majorMatch, priority_score);
              if (item) bao.push(item);
            }
          });
        });
      }
    ...
    console.log('[getCollegeList-expanded] ✅ 兜底结果: 冲=' + chong.length + ', 稳=' + wen.length + ', 保=' + bao.length + ', 总=' + total);
