...
class GaoKaoAnalyzer {
  ...
  getMajorTrend(majorName) {
    for (const industry of INDUSTRY_TRENDS.hot) {
      if (industry.majors.some(m => majorName.includes(m) || m.includes(majorName))) {
  ...
  getCollegeList() {
    ...
    // 遍历所有院校所有专业组
    if (typeof COLLEGES === "undefined" || !COLLEGES) { console.warn("[getCollegeList] COLLEGES未加载"); return { chong: [], wen: [], bao: [] }; } COLLEGES.forEach(college => {
      // 城市过滤
      ...
      college.groups.forEach(group => {
        ...
        if (targetMajor) {
          majorMatch = group.subjects.some(s =>
            s.includes(targetMajor.slice(0, 4)) ||
            targetMajor.includes(s.slice(0, 3))
          );
        }
    ...
    return {
      chong: chong.sort(sortFn).slice(0, 10),
      wen: wen.sort(sortFn).slice(0, 12),
      bao: bao.sort(sortFn).slice(0, 8),
    };
  }
  ...
  buildSummary() {
    ...
    const topChong = colList.chong[0];
    const topWen = colList.wen[0];
    const topBao = colList.bao[0];
